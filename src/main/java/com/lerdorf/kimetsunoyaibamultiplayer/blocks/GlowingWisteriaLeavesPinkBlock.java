...
/**
 * Specialized glowing wisteria leaves for pink variant.
 * Features:
 * - Dim glow effect (light level 4)
 * - Drops pink petal particles when there's air below
 * - Repels monsters (demons) with weakness and slowness effects
 */
public class GlowingWisteriaLeavesPinkBlock extends LeavesBlock {

    public GlowingWisteriaLeavesPinkBlock(Properties properties) {
        super(properties);
    }
        ...
        if (level.isClientSide()) {
            ...
            if (level.isEmptyBlock(below)) {
                ...
                if (random.nextFloat() < 0.1f) {
                    ...
                    // Use dust particles to create a pink effect
                    level.addParticle(
                        new DustParticleOptions(new Vector3f(1.0F, 0.75F, 0.79F), 1.0F), // Pink color
                        x, y, z,
