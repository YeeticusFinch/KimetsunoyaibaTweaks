...
/**
 * Specialized glowing wisteria leaves for cyan variant.
 * Features:
 * - Dim glow effect (light level 4)
 * - Drops cyan petal particles when there's air below
 * - Repels monsters (demons) with weakness and slowness effects
 */
public class GlowingWisteriaLeavesCyanBlock extends LeavesBlock {

    public GlowingWisteriaLeavesCyanBlock(Properties properties) {
        super(properties);
    }
        ...
        if (level.isClientSide()) {
            ...
            if (level.isEmptyBlock(below)) {
                ...
                if (random.nextFloat() < 0.1f) {
                    ...
                    // Use dust particles to create a cyan effect
                    level.addParticle(
                        new DustParticleOptions(new Vector3f(0.0F, 0.8F, 0.8F), 1.0F), // Cyan color
                        x, y, z,
