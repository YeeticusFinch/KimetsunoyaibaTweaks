...
/**
 * Specialized glowing wisteria leaves for lavender variant.
 * Features:
 * - Dim glow effect (light level 4)
 * - Drops lavender/purple petal particles when there's air below
 * - Repels monsters (demons) with weakness and slowness effects
 */
public class GlowingWisteriaLeavesLavenderBlock extends LeavesBlock {

    public GlowingWisteriaLeavesLavenderBlock(Properties properties) {
        super(properties);
    }
        ...
        if (level.isClientSide()) {
            ...
            if (level.isEmptyBlock(below)) {
                ...
                if (random.nextFloat() < 0.1f) {
                    ...
                    // Use dust particles to create a lavender effect
                    level.addParticle(
                        new DustParticleOptions(new Vector3f(0.7F, 0.5F, 0.8F), 1.0F), // Lavender color
                        x, y, z,
