...
/**
 * Specialized wisteria leaves for cream variant.
 * Features:
 * - Drops cream/white petal particles when there's air below
 * - Repels monsters (demons) with weakness and slowness effects
 */
public class WisteriaLeavesCreamBlock extends LeavesBlock {

    public WisteriaLeavesCreamBlock(Properties properties) {
        super(properties);
    }
        ...
        if (level.isClientSide()) {
            ...
            if (level.isEmptyBlock(below)) {
                ...
                if (random.nextFloat() < 0.1f) {
                    ...
                    // Use dust particles to create a cream effect
                    level.addParticle(
                        new DustParticleOptions(new Vector3f(0.95F, 0.9F, 0.8F), 1.0F), // Cream color
                        x, y, z,
    ...
    private void scareNearbyMonsters(Level level, BlockPos pos) {
        ...
        for (LivingEntity demon : entities) {
            ...
            // Apply effects based on distance (NO_DAMAGE)
            if (distance < 3.0) {
