...
/**
 * Tree decorator that places Pink Wisteria Petals hanging from the bottom of leaves
 */
public class WisteriaPetalsPinkDecorator extends TreeDecorator {
    public static final Codec<WisteriaPetalsPinkDecorator> CODEC = Codec.unit(WisteriaPetalsPinkDecorator::new);

    public WisteriaPetalsPinkDecorator() {
    }

    @Override
    protected TreeDecoratorType<?> type() {
        return ModTreeDecorators.WISTERIA_PETALS_PINK.get();
    }
        ...
        context.leaves().forEach(blockPos -> {
            ...
            if (context.isAir(below) && random.nextFloat() < 0.7f) {
                if (level.isStateAtPosition(below, s -> s.isAir())) {
                    ...
                    // Chain length: 0-6 additional blocks
